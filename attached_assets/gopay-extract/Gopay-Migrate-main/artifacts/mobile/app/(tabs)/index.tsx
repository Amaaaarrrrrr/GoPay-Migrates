...
const MENU_ITEMS = [
  ...
  { icon: 'bus-outline'         as const, label: 'PSV Services',  path: '/(tabs)/'     },
  { icon: 'person-outline'      as const, label: 'Profile',       path: '/(tabs)/profile' },
  { icon: 'shield-outline'      as const, label: 'Security',      path: '/(tabs)/profile' },
    ...
    <View style={styles.container}>
      ...
      <Modal visible={menuOpen} transparent animationType="none" onRequestClose={closeMenu}>
        ...
        <Animated.View style={[styles.drawer, { transform: [{ translateX: menuAnim }] }]}>
          ...
          <View style={[styles.drawerFooter, { paddingBottom: insets.bottom + 16 }]}>
            <Pressable style={styles.signOutBtn} onPress={async () => { closeMenu(); await logout(); router.replace('/'); }}>
      ...
      >
        ...
        <View style={styles.cardWrap}>
          <Pressable style={styles.walletCard} onPress={() => router.push('/wallet')}>
            {/* Decorative circles */}
        ...
        <View style={styles.section}>
          ...
          <Pressable style={({ pressed }) => [styles.routeCard, pressed && { opacity: 0.9 }]} onPress={() => router.push('/routes')}>
...
const styles = StyleSheet.create({
  ...
  drawer: {
    ...
    backgroundColor: C.card, borderRightWidth: 1, borderRightColor: C.border, zIndex: 100,
  },
  ...
  walletCard: {
    backgroundColor: C.primary, borderRadius: RADIUS['2xl'], overflow: 'hidden',
    shadowColor: C.primary, shadowOffset: { width: 0, height: 8 }, shadowOpacity: 0.35, shadowRadius: 20, elevation: 8,
  },
